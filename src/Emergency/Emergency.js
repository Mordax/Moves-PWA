import React, { Component } from 'react';
import MenuItem from '../MenuItem/MenuItem';
import { withTranslation } from "react-i18next";

class Emergency extends Component {

    state = {
        dkCallCenter: "",
        maCallCenter: ""
    }

    url = `https://movessw-teamc-baa.herokuapp.com/api/phoneNumber/`;
    
    phoneName = ['dkCallCenter', "maCallCenter"];

    componentDidMount() {
        this.phoneName.forEach(e => {
            fetch(this.url + e, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..uewbDwSp0akoNPK3ljQYTiNjLlW8_5x5uAivDMOZiqM`
                }
            })
            .then(response => response.json())
            .then(res => {
                this.setState({ [e]: res.data[0].formattedNumber });
            });
        });
    }

    render() {
        const { t } = this.props;
        return (
            <div className="container-fluid">
                <div className="row">
                    <div className="col-md-6">
                        <a href={`tel:${this.state.maCallCenter}`}>
                            <MenuItem buttonType="warning" image="/images/morocco.png" title={t('moroccoCallCenter')} subtitle={t('moroccoCallCenter')} />
                        </a>
                    </div>
                    <div className="col-md-6">
                        <a href={`tel:${this.state.dkCallCenter}`}>
                            <MenuItem buttonType="danger" image="/images/denmark.png" title={t('denmarkCallCenter')} subtitle={t('denmarkCallCenter')} />
                        </a>
                    </div>
                </div>
            </div>
        );
    }

}

export default withTranslation() (Emergency);